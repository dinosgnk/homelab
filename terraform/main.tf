...
module "k8s_worker_vm" {
  source = "./modules/proxmox-vm"
  count  = var.k8s_cluster_vms["worker_count"]

  node_name   = "hypatia"
  vm_name     = "${var.k8s_cluster_vms["cluster_name"]}-worker-${count.index + 1}"
  vm_id       = 300 + count.index + 1
  tags        = ["k8s", "worker"]

  hostname       = "${var.k8s_cluster_vms["cluster_name"]}-worker-${count.index + 1}"
  domain         = var.domain
  ssh_public_key = var.ssh_public_key
  ip_address     = "192.168.2.${210 + count.index}/24"
  gateway        = "192.168.2.1"
